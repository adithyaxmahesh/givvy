...
/* ─── Floating icon data — service-type icons ──────────────────── */
  ...
  return (
    <div className="px-4 py-3">
      ...
      <div className="space-y-2">
        {[
          { role: 'Legal Counsel', equity: '0.5%', tag: 'Pre-Seed' },
          { role: 'Lead Engineer', equity: '1.2%', tag: 'Seed' },
        ].map((p) => (
          <div key={p.role} className="bg-gray-50 rounded-lg px-3 py-2">
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-[10px] font-bold text-[#1a1a1a]">{p.role}</span>
              <span className="text-[8px] font-semibold bg-brand-100 text-brand-700 px-1.5 py-0.5 rounded-full">{p.tag}</span>
            </div>
            <span className="text-[9px] text-gray-500">{p.equity} equity via SAFE</span>
          </div>
        ))}
      </div>
    </div>
